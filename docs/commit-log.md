...
- 2026-07-15 10:52 — Fix two shared-pool coherence bugs the France screenshot exposed: strip 33 culture-specific terms (提親/衙門/科舉/束脩/算盤/扁擔/祠堂…) from the 9 globally-shared narrative pools that all 196 countries draw from, keeping them only in the eastAsia region table where they belong; and break the death-cause monotony (瘟疫 24%→6% of pre-1750 rolls, 霍亂 29%→8%, 心臟病 30%→17%) by diversifying every era into 30+ distinct period-accurate causes. Locked in with test_shared_pool.js- 2026-07-15 11:16 — Draw real country borders on the globe (Natural Earth 110m admin-0, 333 polylines clipped per-point at the horizon) and rebuild the THE END sunset: mountains were squashed flat because a 100-unit viewBox with preserveAspectRatio="none" crushed the ridge amplitude into 5.8% of screen height, so peaks now use the full viewBox across three atmospheric ranges, plus a backlit rim light and fixes for the sun's hard-edged glow disc and the rays' visible circular boundary
- 2026-07-17 09:40 — Add per-review shuffle bags so no story line repeats within one 10-life chain review (random picks were colliding: ~7 marriages drew from a 12-line pool), and start the 5x text expansion with MEET_SPOUSE 12→36 written for emotional weight
...
- 2026-07-26 00:27 — Thread rewrite batch 2 (final): remaining 12 occupation categories become short thread-ordered life-lines (anchor in childhood, payoff in twilight); add {crop} flavor token; convert all em-dashes to spaced hyphens per user preference; all 14 categories index-aligned and passing tests
- 2026-07-26 01:43 — Add era×region tone layer: occupation verb / passing verb / death-cause connector now vary by era band (度日→討生活, 歿於→死因是) and region imagery (歸了塵土/了了此生/與世長辭), framing-only, narrative pools untouched
